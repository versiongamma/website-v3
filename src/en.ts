export const en = {
  landing: {
    title: "Hey! I'm Matt",
    message: {
      part1:
        'I make things! Professionally I make software, non-professionally I make photos and videos, and amateurly I do electronics, 3D printing & CAD, woodworking, and loud noises with drums.',
      part2: 'If you’d like to learn more, you can at the links below:',
    },
  },
  videos: {
    youtube: {
      title: 'Version Gamma',
      description:
        ' I make videos under my channel Version Gamma, in which I mostly focus on analysing media such as video games, shows, or really anything I find interesting.',
    },
    videography: {
      title: 'Videography',
      description:
        "As a video editor & cinematographer, there's a bunch of stuff I've worked on or made that isn't part of my YouTube channel. Music videos for my brother's work as Army of One, various short films I've had a hand in, and a lot of just dicking around with a camera.",
    },
  },
  photos: {
    description: {
      part1:
        "Welcome to the gallery of photo's I've taken over the years! As you'll notice, I mostly do landscape and setting photography, as I happen to have been graced by getting to live in the beautiful country of Aoteroa. I'm not much for taking photos for social media on trips, but I do try and get a good photo or two.",
      part2:
        'I shoot with FujiFilm cameras, previously an X-T3 and currently an X-H2S; using the Fuji 16-55mm f2.8 lens for photography, and the various Fuji f1.4 primes for video, mainly the 33mm.',
      part3:
        'And obviously, the film photos are shot on a 35mm film camera, with Ilford B&W stocks and Fuji colour stocks.',
    },
  },
} as const
